...
\begin{lstlisting}
testonuevo2 = " ".join([w for w in testo.split() if w not in stopwords])
\end{lstlisting}
\pause
This is more efficient and more ``pthonic'', but may be more difficult to debug (especially if it gets more complicated)
...
Or:
\begin{lstlisting}
print(" ".join([stemmer.stem(p) for p in frase.lower().split() if p not in stopwords]))
\end{lstlisting}
