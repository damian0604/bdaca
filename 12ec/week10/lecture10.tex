...
\color{blue}Logistic regression estimates $\beta$-coefficients by minimizing the ``cost'' that occurs when the prediction is wrong.
...
\begin{itemize}[<+->]
	...
	\item Remember that we are \textit{not} interested in a substantive interpretation of the  $\beta$-coefficients but in a good prediction (which is why ``punishing'')
	\item The penalty will shring the coefficients towards zero, \emph{which is very useful to avoid overfitting!}
