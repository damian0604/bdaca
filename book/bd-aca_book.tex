...
\date{Version 1.0\\ ~ \\  \footnotesize{PDF created \today}}
...
Copyright \copyright\ 2015, 2016, \the\year\ Damian Trilling\\
\vspace{.25cm} \\
...
Pay also attention to the line numbering at the left side of the code examples: It helps you see whether a line really ends (and that you should press enter), or whether it the paper is just too small to print the line on one line:
...
\begin{itemize}
	\item When you are asked how much ``base memory'' you want to allocate to your machine, you should choose a value of at the \emph{very} least around 1000 MB. More is better, if you you have 4,096 MB of RAM in total, you can safely choose 2,048 MB. The examples in this tutorial are tested on a VM with 2,048 MB RAM. 
...
After finishing the installation, your virtual machine will reboot. After clicking on reboot, you can get a message saying ``please remove installation disks''. You can just press enter, you don't have to remove anything in your case. You might have to press enter a second time.
...
\end{lstlistingbash}
You will be asked for your password (the one you defined during the install process). Just type it and confirm with Enter (no, you do not see anything while you type, that's correct). You will see a long list of packages to be installed, which you -- as the last line shows -- have to confirm with yes (by typing the letter \texttt{y}) followed by Enter). 
...
And let us finally install some additional Python modules (by the way, you can use the arrow-up key to get the previous command you typed):
...
Make also sure that you have the correct version of IPython installed -- if it is too old, then you might have problems later on, because spyder and/or jupyter-notebook might not be fully compatible. Check the version from the Linux command line:

\begin{lstlistingbash}
ipython3 --version
\end{lstlistingbash}

The version number should be higher than 5. In case it is not, try the following:
...
From the Selenium package, you must then import some modules in your Python script: 
...
In addition to finding elements by their class name, it is possible to find them using other classifiers as well (for example, by their Xpath!). For more information about different manners to detect elements using Selenium, see
\url{http://selenium-python.readthedocs.io/locating-elements.html}.

Now, let’s enter a search word in the search field. For this, we use the Keys module that we imported earlier, because this allows us to automate everything that a human would type with the keyboard.

\begin{lstlisting}
element.send_keys("TinTin")
\end{lstlisting}

Now, we can use class names again to detect the ``search'' button and then instruct the computer to click it. However, we can also just press ‘enter’ instead of clicking ``search'', as many humans also do. For this we use the Keys module gain. 
...
In line 2, we instruct the computer to wait 10 seconds before it does anything. This way, we provide some time for the page to load. This can also be useful to conceal the fact that there is no human, but in fact an automated script accessing the content/

In addition to letting the computer wait for some seconds, it is also possible to let the driver wait until a specific element has appeared on a website (this is called an explicit wait). This can be nice for example if you want to click a button of which you know that it takes a while before it appears. Take a look at this webpage if you want to learn more about waits: \url{http://selenium-python.readthedocs.io/waits.html}.

We have now reached the first page with the Google results. Let's take a screenshot of this page. When we do this, we have to specify where we want the computer to save the screenshot and how we want the screenshot to be named.

\begin{lstlisting}
driver.save_screenshot("/home/marthe/images/screenshot_page1") 
\end{lstlisting}

Let's go to the next page by clicking ``next'' at the bottom of the page. First, we must again tell the computer what element of the webpage we want it to click. We can again define the element by looking for its class name. 

\begin{lstlisting}
element2 = driver.find_element_by_class_name("pn")
element2.click()
driver.implicitly_wait(10)
\end{lstlisting}

When we are done, we can close the browser by typing:
