...
\vspace{.25cm} \\
This document can be obtained from SSRN URL OR FIGSHARE URL. The source code of the most recent version can be found at \url{https://github.com/damian0604/bdaca}.
...
\begin{quote}
At the time of writing, the most recent version is 15.10, and that’s what you download. If you should be using an older Apple computer that still has a PowerPC rather than an Intel processor  (the white iBooks, for example, produced until 2006), than you will have to use version 12.04 instead (\url{http://cdimages.ubuntu.com/lubuntu/releases/precise/release/lubuntu-12.04-desktop-powerpc.iso}). Note that in that case, all following steps might be slightly different.

\end{quote}


\section{Create a virtual machine}
Open VirtualBox, click on ``New'', give your image a name, and select first Linux, then Ubuntu~64bit from the list (or 32 bit, if that's what you downloaded). On the following screens, just leave the default settings and agree to everything---with one exception: When you are asked how much ``base memory'' you want to allocate to your machine, you should choose a value of at least around 1000 MB. More is better, if you you have 4,096 MB of RAM in total, you can safely choose 2,048 MB. The examples in this tutorial are tested on a VM with 2,048 MB RAM. 
...
We will now install some other programs we will use with the following commands
(if you are running the 32bit version, leave away the \texttt{\_64}). Just type them into the LX Terminal, which you already know:

\begin{lstlisting}
sudo apt-get install gedit spyder3
\end{lstlisting}

You will be asked for your password and some questions which have to be answered with yes (by typing the letter \texttt{y}) followed by Enter. And another two commands to type for installing additional Python modules:
...
%\section{Done!}
%You can also have a closer look at all the options VirtualBox has to offer, for example about sharing folders or the clipboard between the host system (your ``real'' system) and the virtual system (Ubuntu, in this case). That's all not strictly necessary for this course, but as it is \emph{your} working environment for the next time, you might as well make it suit your needs as closely as possible.
...
\end{lstlisting}
(Of course, you replace USERNAME and PASSWORD by the username and password you received from your instructor to access all class materials. Also note that they are entered in the same line as the rest of the \texttt{wget}-command. \textbf{And note the space before the \texttt{--}!}).
...
\chapter{The very, very basics of programming}
\label{chap:basics}
...
If you just type \texttt{python3} somewhere in the command line, you can actually also do the following examples (otherwise, you have to trust me that this is how it works). When you are finished with the exercises, just type \texttt{quit()}.

Alternatively, and to enjoy more comfort, you can use a graphical interface called: just type \texttt{spyder3} instead of \texttt{python3}. 

Another way that is often used is to write a Python program in an arbitrary text editor, save it, and then start the program you wrote from the command line. The fact that there are so many ways to issue Python commands is actually a first important lesson: Python is not a program (like SPSS, or Word or Excel are programs), but a language --- and there are a bunch of programs one can talk to in this language. Because of this, it is so flexible and can be used in so many contexts.
...
\ldots \emph{string}, the fourth variable. A string contains some arbitrary text, which is indicated by surrounding \texttt{" "} or \texttt{' '}.
...
To tell the computer to repeat something, we use a construction called a loop, or more specifically, a \emph{for-loop}. Lets start with an example (when typing it over, start line 3 by pressing SPACE four times):
...
\begin{quote}
After a line ending with a colon, Python expects an indented block. If you do not do this in a consequent way (for example, you sometimes press TAB, sometimes use 4 spaces, sometimes 3, or none at all, Python will complain and give you an ``indention error''.
\end{quote}
...
You actually already saw a CSV file on page \pageref{firstcsvfile}. Have a look back there if you forgot. Virtually all programs (including SPSS, Stata, Excel, \ldots) can read and write CSV files, that's why we will use them very frequently. When creating data with such a program, make sure that you know how they are formatted exactly (e.g., are columns separated by \texttt{,} or \texttt{;}, are strings encapsulated by \texttt{""} or not, \ldots. You can use the tools described on \pageref{firstcsvfile} to inspect the files.
...
Let's now save these columns in to a new file, \texttt{test.csv}, but in a different order. To this end, we use the \texttt{zip} command that basically clues several lists together. It goes like this:
...
Lets again with an editor like gedit and create a file with the following content and call it \texttt{/home/damian/mensen.json}.
\begin{lstlisting}
...
For this exercise, we need to import a module named \texttt{twitter}. You can easily install it from the command line with
\begin{lstlisting}
...
for i in posts:
    ...
    dates.append(posts[i]["created_at"])
...
However, in a real-world setting, it might be too simplistic. Instead of writing a sentiment analysis algorithm yourself, you can therefore invoke some third-party algorithm like SentiStrength \cite{Thelwall2012}. SentiStrength is free for academic use, but you have to pay for it if you want to use it commercially. To link it to Python, I wrote a module called \texttt{sentipy}, which you already installed. You can test wether everything works:
...
This is pretty useful, because we can address the items with \emph{slicing}. For example, to get the fifth tuple, we can write \texttt{tagged[5]}. Of course, we can also slice the tuple. So, to get only the grammatical function of the word ``morning'', we can use  \texttt{tagged[5][1]}.
...
\end{lstlisting}
\label{greedy}
Pay attention that we write \texttt{.*?} instead of \texttt{.*} only. The \texttt{.} matches every character, and \texttt{*} means an arbitrary number of times, therefore \texttt{.*} \emph{also} matches \texttt{</div>}. This means that our regular expression would never stop, and we would get the whole rest of \texttt{tekst} --- rather the part \emph{until \texttt{</div>} is reached}. This is referred to as \emph{greedy matching}. Using \texttt{.*?} instead of \texttt{.*} means that we want to perform \emph{lazy matching} instead, where we do stop as soon as the next part of the regular expression is matched.
...
One does not always have  to reinvent the weel, though. It is incredibly useful to understand how parsing with regular expression works, and it is very likely to come into a situation where this knowledge can be applied. Especially in the case of HTML pages, though, several parsers exist that can make life easier. 
...
\begin{itemize}
\item If you want to refer to a specific attribute of a HTML tag, you can use \texttt{@}. For example, every \texttt{*[@id="reviews-container"]} in an XPath would grap what is within a tag like \texttt{<div id="reviews-container" class="user-content">}
\end{itemize}
...
You can get very far with the approaches sketched in this paper: regular expressions, the lxml-package, and wget for automated downloading tasks. BeautifulSoup is a python package that lies somewhere in between, as it resembles lxml, but internally uses regular expressions rather than an XPath to parse the HTML. 

If you want to write an implication that integrates crawling and parsing, you might have a look at the scrapy framework (\url{http://scrapy.org/})
...
In particular, we will look how we can construct a network of words that co-occur together in texts, which can be a nice way to conduct some inductive framing analysis \citep[see][]{Boumans2016}.
...
The GDF file your program produces can be opened in Gephi (or some other network visualization or analysis tool). I will provide you with one solution, but of course you can also try yourself!
...
Topic Modelling is one form of unsupervised machine learning.

You can use gensim \cite{Rehurek2010} for this. Install the module from the bash terminal:

\begin{lstlisting}
sudo pip3 install gensim
\end{lstlisting}

You might need to restart the Python console you are using (or just restart Spyder).
...
\todo[inline]{verwijzen naar website numpy/scipy en ook citeren! }
...
\section{pandas}
\todo[inline]{naar boek verwijzen}
Pandas is a framework that allows statistical modelling in Python. Those of you who know R will see a lot of similarities. Within pandas, data are stored in a table, very much like a Stata or SPSS dataset -- and yes, it is indeed called exactly as it is called in R: a \emph{dataframe}.
As you see below, a dataframe basically consists of different columns with a name, which consist of a list of data. Pandas can read directly fromm a CSV file, but as you can see below, you can also construct a dataframe yourself. To run an OLS regression, for example you then only have to specify which columns are the dependent and the independent variables. 

\begin{lstlisting}
import pandas as pd
from pandas.stats.api import ols
df = pd.DataFrame({"y": [10,20,30,40,50], "x1": [20, 30, 10, 40, 50], "x2": [32, 234, 23, 23, 42523]})
result = ols(y=df['y'], x=df[['x1','x2']])
print(result)
\end{lstlisting}
...
--------------------------------------------------------------------------
             x1     0.4012     0.6497       0.62     0.5999    -0.8723     1.6746
             x2     0.0004     0.0005       0.65     0.5826    -0.0007     0.0014
     intercept    14.9525    17.7643       0.84     0.4886   -19.8655    49.7705
---------------------------------End of Summary---------------------------
\end{lstlisting}

Of course, you might want to give your variables more meaningful names than \texttt{x1} and \texttt{x2}.

A nice extra is that you can get much more than this regression table. You can access the estimates directly to do further calculations on them. Or, pretty cool, you easily get a list of predicted values with (\texttt{result.y\_predict}).
...
While there are tools that make fancier graphics (take a look at the Python module \texttt{seaborn}; or use \texttt{ggplot2} in R), \texttt{matplotlib} is really useful if you just want to make a quick histogram, line graph, or scatter plot (Fig.~\ref{fig:matplotlib}).
...
The following books provide the interested student with more and deeper information. They are intended for the advanced reader and might be useful for individual projects:
...
\todo[inline]{ADD MORE RESOURCES}
\todo[inline]{CHECKEN OF IK HET OP SSRN GA ZETTEN (IVM VERSIONING) OF GITHUB MISSCHIEN -- ja, zeker doen, en dan erop wijzen (``most recent version can be found on github''}
